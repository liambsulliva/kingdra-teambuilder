...
export default function Component({ pokemonParty, setPokemonParty }: { pokemonParty: pokemon[], setPokemonParty: React.Dispatch<React.SetStateAction<pokemon[]>> }) {
  ...
  const parsePokemonParty = async (data: string): Promise<pokemon[] | null> => {
    const pokemonStrings = data.split('\n\n').filter(str => str.trim() !== '');
